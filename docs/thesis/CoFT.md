...
- **Chapter 4: RESULTS: THE JOURNEY OF DISCOVERY**
...
- **Chapter 5: SUMMARY AND REFLECTION**
  ...
  - 5.3 Key Findings and Scientific Insights
    - 5.3.1 The "Less is More" Discovery: A New Perspective on Co-Training
    - 5.3.2 The Value of Simplicity: An Augmentation Case Study
...
2.  **The "Less is More" Discovery and "Label Confusion" Theory:** We systematically discovered that an ultra-low co-training weight (`lambda_ct` = 0.0001) is optimal. We provided a theoretical explanation, the "Label Confusion Theory," for why high coupling weights degrade performance, a counter-intuitive finding with significant implications for future co-training research.
3.  **State-of-the-Art Performance on Diverse Datasets:** We demonstrated the effectiveness of CoFT on three public benchmark datasets (HAR, Sleep-EDF, Epilepsy), achieving substantial performance improvements over strong baselines, particularly in low-label scenarios (e.g., a **+8.17%** absolute accuracy gain on HAR with 1% labels).
4.  **A Principled Parameter Transfer Methodology:** We developed and validated a scientific approach for adapting optimized hyperparameters to new datasets based on their intrinsic signal characteristics (e.g., sequence length, signal-to-noise ratio), dramatically reducing the need for costly, exhaustive grid searches.
...
Artificial Intelligence (AI) tools were utilized in this thesis as an aid to the research and writing process. Specifically, Large Language Models (LLMs) were employed for tasks such as:

*   **Literature Discovery:** Assisting in summarizing articles and identifying relevant research papers during the literature review phase.
*   **Code Generation and Debugging:** Generating boilerplate code snippets for the experimental framework in PyTorch and assisting in debugging implementation issues.
*   **Content Structuring and Rephrasing:** Helping to organize the structure of the manuscript and rephrasing sentences and paragraphs to improve clarity, grammar, and flow.

All core concepts, experimental designs, data analysis, and scientific conclusions presented in this thesis are the original work of the author. AI tools served as a productivity assistant and were not used to generate the novel contributions or scientific insights of this work.
...
The hybrid loss function represented one of the most challenging design aspects of CoFT, requiring careful balance between multiple competing objectives while maintaining numerical stability. After extensive experimentation, the final loss formulation carefully balanced domain-specific and cross-domain objectives:
...
For the frequency branch, a similarly straightforward approach was adopted to establish a robust baseline. The augmentation was applied directly to the time-domain signal *before* the Fast Fourier Transform (FFT) was computed.

-   **Time-Domain Jitter:** The primary augmentation for the frequency view was also `jitter`. By adding noise in the time domain, we introduced broad-spectrum, low-amplitude perturbations in the frequency domain. This simple method proved effective in creating a sufficiently different yet correlated view for the contrastive task, allowing the model to learn robust spectral features without the need for more complex, direct spectral manipulations in this initial version of the framework.

---

### **4 RESULTS: THE JOURNEY OF DISCOVERY**
...
**4.4.2.1 HAR Dataset: Breakthrough Success**
HAR represented the "ideal" scenario for CoFT, showing a massive **+8.17%** absolute improvement in the 1% label case. This was a direct result of the frequency domain providing rich, complementary information about human movement patterns (e.g., step cadence, orientation signals) that the temporal domain alone could not capture as effectively.

**4.4.2.2 Sleep-EDF: Outstanding Accuracy Gains and a Key Insight into F1-Score Dynamics**
Sleep-EDF showed the most dramatic accuracy improvements (**+9.32%** for 1%, **+8.63%** for 5%). These results were achieved using parameters transferred from HAR, not through a full, dataset-specific optimization.

A crucial finding from the Sleep-EDF experiments was the behavior of the Macro F1-score, which did not improve in line with accuracy. This was not a shortcoming of the framework but rather a significant insight into its interaction with heavily imbalanced data. The F1-score's sensitivity to under-represented classes (like N1, REM) highlighted that while CoFT dramatically improved overall classification accuracy, achieving optimal performance across all minority classes required dataset-specific parameter tuning. This result reinforced the validity of the parameter transfer methodology (Section 4.2.6) as a tool for robust initial adaptation, while simultaneously underscoring the necessity of targeted optimization for class-balanced performance, a primary goal for future work.

**4.4.2.3 Epilepsy Dataset: Strong Gains Under Difficult Conditions**
Epilepsy showed consistent accuracy improvements, particularly the **+2.71%** gain in the challenging 1% label scenario. Like with Sleep-EDF, these results used parameters transferred from HAR. This strong performance, despite class imbalance and different signal characteristics, further proved the framework's robustness.

##### **4.4.3 Cross-Dataset Pattern Analysis**

**4.4.3.1 Label Percentage Effects**
Consistent pattern across all datasets: **CoFT improvements were larger with fewer labels.**
...
**Interpretation**: The frequency domain provided the most value as a regularizer and information source when labeled data was scarce. As more labels became available, the benefit diminished, though it remained positive.

**4.4.3.2 Domain Characteristics and CoFT Effectiveness**

**High Effectiveness (Sleep-EDF, HAR):**
- **Multi-class problems**: More classes benefited more from cross-domain information
- **Rich spectral content**: Clear frequency patterns complemented temporal features
...
- **Highly imbalanced**: Majority class dominance limited learning
- **Strong temporal patterns**: Seizures had clear temporal signatures
...
This case study powerfully reinforces the main thesis: the value of CoFT is not just in boosting a single accuracy number, but in creating a more robust and nuanced model. For medical applications, improving the detection of rare but critical events (reflected by F1-Macro) is often more valuable than a marginal increase in overall accuracy.
...
This thesis presented **CoFT (Co-training with Frequency and Temporal domains)**, a novel framework that successfully bridged the gap between frequency and temporal domain analysis in time series classification. Through rigorous experimentation and optimization, we achieved significant performance improvements while uncovering fundamental insights about cross-domain learning.
...
3. **The "Less is More" Discovery**: Uncovered that an ultra-low lambda_ct value (0.0001) was optimal, challenging conventional wisdom and leading to the "Label Confusion Theory".
4. **Parameter Transfer Methodology**: Developed a scientific approach to adapt hyperparameters to new datasets based on signal characteristics, reducing the need for costly, exhaustive grid searches.

**Scientific Discoveries:**
1. **Label Confusion Theory**: A clear explanation for why high lambda_ct degraded performance in semi-supervised settings.
2. **Complexity vs. Benefit Analysis**: Proved that simple, efficient data augmentations delivered better ROI than complex alternatives like InfoTS.
3. **Synergistic Domain Contribution**: Empirically verified that at the optimal configuration, the frequency branch acted as a synergistic contributor, not a performance bottleneck.
...
##### **5.3.1 The "Less is More" Discovery: A New Perspective on Co-Training**
The journey to find the optimal lambda_ct was a three-week-long investigation that systematically dismantled our initial intuitions. Conventional wisdom suggested a strong coupling was key; our results, however, whispered a different, much quieter secret: 'less is more'. This challenged conventional wisdom and provided a new framework for understanding co-training dynamics: *High lambda_ct created "label confusion" where pseudo-labels conflicted with ground truth, degrading performance. Ultra-low weights provided gentle regularization without overwhelming the ground-truth signal.*

##### **5.3.2 The Value of Simplicity: An Augmentation Case Study**
Our systematic comparison of augmentation strategies provided clear guidance for research resource allocation: *In time series research, complexity should be justified by performance gains. Simple, interpretable methods often provided better ROI than sophisticated alternatives.*
...
1.  **Domain-Specific Architectures for the Frequency Branch:** Our ablation studies and performance analyses suggested that the frequency branch, while beneficial, had not yet reached its full potential. The mirrored architecture served as a crucial controlled baseline, but a key future direction was to explore and design **specialized neural architectures** tailored specifically for learning from frequency-domain representations, moving beyond the current general-purpose encoders.

2.  **Advanced Interaction and Fusion Mechanisms:** The current co-training mechanism relied on a gentle, regularizing signal. Future work could investigate more **dynamic and fine-grained interaction mechanisms** between the two branches. Exploring methods like attention-based fusion or mutual knowledge distillation could foster a deeper level of synergy, allowing the branches to collaborate more intelligently.

3.  **Augmentation-Free Learning for Sensitive Data:** As discussed in Section 5.3.3, applying contrastive learning to sensitive clinical data required careful consideration of data augmentation. A significant research direction was the development of **augmentation-free self-supervised learning** paradigms. Frameworks based on cross-domain prediction or reconstruction (e.g., using one domain's representation to predict the other's) could provide a powerful learning signal without the risk of distorting diagnostically critical patterns.

4.  **Application to Broader Time Series Tasks:** Beyond classification, the robust, disentangled representations learned by CoFT could be highly beneficial for other time series tasks, such as **long-term forecasting and anomaly detection**, which remained a promising area for exploration.
