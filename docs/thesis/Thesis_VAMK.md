...
The proliferation of time series data across numerous domains, from industrial IoT to healthcare, has been met with a critical bottleneck: the scarcity of labeled data required for training robust deep learning models. This "data rich, label poor" exigency is particularly pronounced in high-stakes fields like medicine, where annotating signals such as EEGs or ECGs is expensive, time-consuming, and requires deep domain expertise. This thesis directly confronts this label scarcity challenge, aiming to unlock the potential of vast unlabeled data reserves.

We propose **CoFT (Co-training with Frequency and Temporal domains)**, a novel dual-branch, semi-supervised learning framework. CoFT uniquely operationalizes the temporal and frequency domains not as features to be fused, but as two conditionally independent "views" for a true co-training methodology. The research utilizes a sophisticated six-stage training pipeline built upon the state-of-the-art CA-TCC framework, extending it with a parallel frequency-domain branch and a cross-domain co-training module orchestrated by a hybrid loss function. To ensure scientific rigor, experiments were conducted on three public benchmark datasets—HAR, Sleep-EDF, and Epilepsy—with a painstaking replication of the original data preprocessing and splitting methodology to guarantee academic fairness.
