...
def data_generator(data_path, configs, training_mode, enable_coft=False):
    ...
    if "_1p" in training_mode:
        train_dataset = torch.load(os.path.join(data_path, "train_1perc.pt"), weights_only=False)
        ...
        train_dataset = torch.load(os.path.join(data_path, "train_10perc.pt"), weights_only=False)
    elif "_50p" in training_mode:
        train_dataset = torch.load(os.path.join(data_path, "train_50perc.pt"), weights_only=False)
