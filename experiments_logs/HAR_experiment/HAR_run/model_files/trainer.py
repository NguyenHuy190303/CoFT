...
def Trainer(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, train_dl, valid_dl, test_dl, device,
            logger, config, experiment_log_dir, training_mode, amp_enabled=True):
    # Start training
    ...
    for epoch in range(1, config.num_epoch + 1):
        ...
        train_loss, train_acc = model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer,
                                            criterion, train_dl, config, device, training_mode, scaler, amp_enabled)
        valid_loss, valid_acc, _, _ = model_evaluate(model, temporal_contr_model, valid_dl, device, training_mode, amp_enabled)
    ...
    if (training_mode != "self_supervised") and (training_mode != "SupCon"):
        ...
        logger.debug('\nEvaluate on the Test set:')
        test_loss, test_acc, _, _ = model_evaluate(model, temporal_contr_model, test_dl, device, training_mode, amp_enabled)
...
def model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_loader, config,
                device, training_mode, scaler, amp_enabled):
    total_loss = []
    total_acc = []
    model.train()
...
def model_evaluate(model, temporal_contr_model, test_dl, device, training_mode, amp_enabled):
    model.eval()
...
def gen_pseudo_labels(model, dataloader, device, experiment_log_dir, amp_enabled=True):
